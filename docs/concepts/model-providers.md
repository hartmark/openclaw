...
- The `"sse"` transport also supports HTTP continuation for a native `openai/openai-responses` model at the exact public `https://api.openai.com/v1` base URL (ChatGPT/Codex `openai-chatgpt-responses` routes are excluded and deliberately stay `store: false`): OpenClaw caches the request/response baseline per session+credential in process memory and, when the next turn's history is a strict extension, sends only the new input plus `previous_response_id` instead of the full growing history. That baseline evicts after `compat.responsesContinuationIdleMinutes` (default 90) minutes without a turn; a gap longer than that, or a rejected/expired `previous_response_id` (Zero Data Retention, TTL eviction), retries once, same turn, with the full request and starts a fresh baseline.
  - On a custom/proxy OpenAI-Responses-compatible endpoint (self-hosted router, gateway, etc.), the host itself carries no trust signal — set `compat.supportsResponsesContinuation: true` on that model to opt it in once you've verified the backend correctly resolves `previous_response_id` and persists `store: true` turns. This is the _only_ path to eligibility for a custom endpoint; it stays off for every other model on that connection, and it never overrides an explicit `store: false`. A configured `compat.responsesContinuationIdleMinutes` override currently has no effect on a model that resolves to a known catalog route (including the native OpenAI route documented above) — catalog-owned `compat` takes precedence over configured `compat` for those routes, so the override is reachable only on a custom/proxy endpoint today.
...
- Native OpenAI routes also keep Responses `store`, prompt-cache hints, and OpenAI reasoning-compat payload shaping; proxy routes do not, except a model with the explicit `compat.supportsResponsesContinuation: true` opt-in above, which forces `store: true` on that one model to get HTTP continuation working
