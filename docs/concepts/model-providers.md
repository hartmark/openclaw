...
- The `"sse"` transport also caches a request/response baseline per session in process memory so the next turn can send only new input plus `previous_response_id` instead of the full growing history. That baseline evicts after `compat.responsesContinuationIdleMinutes` (default 90) minutes without a turn; a gap longer than that resends full history once and starts a fresh baseline. This override currently has no effect on a model that resolves to a known catalog route (including the native OpenAI route documented here) — catalog-owned `compat` takes precedence over configured `compat` for those routes, so the override is reachable only on a custom/proxy endpoint today.
