...
- The `"sse"` transport also caches a request/response baseline per session in process memory so the next turn can send only new input plus `previous_response_id` instead of the full growing history. That baseline evicts after `params.compat.responsesContinuationIdleMinutes` (default 90) minutes without a turn; a gap longer than that resends full history once and starts a fresh baseline.
