...
| `headers`             | object (string → string) | Extra headers sent with every request, e.g. `Authorization` for a ComfyUI instance behind HTTP auth. Merged over the plugin's own headers; a key set here always wins. |
