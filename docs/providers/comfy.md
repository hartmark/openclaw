...
| `headers`             | object (string → string or [secret reference](/gateway/configuration-reference#secrets)) | Extra headers sent with every request, e.g. `Authorization` for a ComfyUI instance behind HTTP auth. Each value accepts a literal string or a SecretRef, same as `apiKey`. Merged over the plugin's own headers; a key set here always wins, except a configured `Content-Type` is ignored on image-upload requests specifically, since the multipart form boundary there must come from the runtime. A header pointed at an unresolvable SecretRef fails the request rather than sending it unauthenticated. |
