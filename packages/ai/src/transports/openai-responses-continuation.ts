...
function restoreRawCallIdsInDelta(
  ...
  if (rawCallIdByReshaped.size === 0) {
    // SAFETY: only drops the `readonly` modifier; the array's own contents are untouched.
...
export function resolveResponsesContinuationRequest(
  ...
  return {
    request: {
      ...
      // SAFETY: only rewrites call_id strings in place; item shapes already satisfy ResponseInput.
      input: restoredInput as ResponseInput,
    },
