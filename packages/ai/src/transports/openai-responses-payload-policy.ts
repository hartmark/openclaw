...
function readCompatPayloadBoolean(
  ...
  // SAFETY: already narrowed to a plain object above; only re-indexes it by a caller-supplied key.
...
function readCompatContinuationIdleMinutes(compat: unknown): number {
  ...
  // SAFETY: already narrowed to a plain object above; the numeric shape is validated below.
